...
@property (strong, nonatomic) UISegmentedControl *scaleControl;
@property (strong, nonatomic) UISegmentedControl *payTypeControl;
@property (strong, nonatomic) UISegmentedControl *visibilityControl;
...
@property (strong, nonatomic) NSArray *typeSelectedImages;
...
@property (nonatomic) NSInteger galleryTag;
@end
...
- (void)viewDidLoad {
    ...
    self.typeSelectedImages = @[@"type_studyGroup.png", @"type_reviewSession.png", @"type_partnerRecruiting.png", @"type_infoSession.png", @"type_other.png"];
...
- (void)changeHeaderImage:(NSInteger)tag {
    switch (tag) {
        case 0:
            self.backgroundImage = [UIImage imageNamed:@"study group"];
            break;
        case 1:
            self.backgroundImage = [UIImage imageNamed:@"review session"];
            break;
        case 2:
            self.backgroundImage = [UIImage imageNamed:@"partner recruiting"];
            break;
        case 3:
            self.backgroundImage = [UIImage imageNamed:@"info session"];
            break;
        case 4:
...
- (void)configureTableHeaderView {
    ...
    imageView.image = [UIImage imageNamed:@"study group"];
    imageView.userInteractionEnabled = YES;
    UITapGestureRecognizer *tapRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapHeaderImageView:)];
...
- (void)configureTableFooterView {
    ...
    UIButton *inviteButton = [[UIButton alloc] initWithFrame:CGRectMake(10.0, 10.0, SCREENWIDTH / 2 - 20.0, 50.0)];
    [inviteButton addTarget:self action:@selector(goInviteFriend) forControlEvents:UIControlEventTouchUpInside];
    [inviteButton setBackgroundImage:[UIImage imageNamed:@"btn_red.png"] forState:UIControlStateNormal];
    [inviteButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [inviteButton setTitle:@"invite" forState:UIControlStateNormal];
    inviteButton.titleLabel.font = [UIFont boldSystemFontOfSize:17.0];
...
- (void)tapHeaderImageView:(UIGestureRecognizer *)gestuerRecognizer {
    GalleryViewController *galleryController = [[GalleryViewController alloc] init];
    galleryController.delegate = self;
    self.galleryTag = 0;
    UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:galleryController];
...
- (void)chooseImageButtonDidClick:(UIButton *)button {
    GalleryViewController *galleryController = [[GalleryViewController alloc] init];
    galleryController.delegate = self;
    self.galleryTag = 1;
    UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:galleryController];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (row == 0) {
        ...
        if (cell == nil) {
            ...
            for (NSInteger i = 0; i < 8; i++) {
                ...
                UIButton *button = [UIButton eventTypeButtonWithFramg:CGRectMake(buttonWidth * (i % 4), originY, buttonWidth, buttonHeight)
                                                       ...
                                                       highlightImage:[UIImage imageNamed:_typeSelectedImages[i]]
                                                                 text:_typeTitles[i]];
                button.tag = i;
        ...
        if (cell == nil) {
            ...
            [cell.contentView addSubview:label];
            NSArray *items = @[@"apply/confirm needed", @"join in directly"];
...
#pragma mark -- GalleryControllerDelegate
- (void)galleryController:(GalleryViewController *)controller didChooseImages:(NSArray *)images forIndex:(NSArray *)list {
    if (_galleryTag == 0) {
        ALAssetsLibrary *library = [[ALAssetsLibrary alloc] init];
        [library assetForURL:[NSURL URLWithString:images[0]]
                 resultBlock:^(ALAsset *asset) {
                     UIImage *image = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullResolutionImage]];
                     //                     _headerImageView.image = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullResolutionImage]];
                     //                     self.backgroundImage = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullResolutionImage]];
                     CGRect cropFrame = CGRectMake(10.0, 200.0, SCREENWIDTH - 20.0, (SCREENWIDTH - 20.0) * 150 / 320);
                     VPImageCropperViewController *cropController = [[VPImageCropperViewController alloc] initWithImage:image
                                                                                                              cropFrame:cropFrame
                                                                                                        limitScaleRatio:3.0];
    ...
    } else if (_galleryTag == 1) {
        self.eventImages = [images mutableCopy];
...
- (void)imageCropper:(VPImageCropperViewController *)cropperViewController didFinished:(UIImage *)editedImage {
    ...
    self.backgroundImage = editedImage;
    [cropperViewController.navigationController dismissViewControllerAnimated:YES completion:^{
